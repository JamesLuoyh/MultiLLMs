...
class WeightedScoreWagers(WageringMethod):
    ...
    def compute_wagers(
        ...
        **kwargs
        ...
        Returns:
            Dictionary with "wagers": [batch_size, num_models]
        """
    ...
    def update(
        ...
        **kwargs
        ...
        with torch.enable_grad():
            ...
            # scores = scores - 0.5 * average_scores.detach()
            
            # net_payout = sigmoid_wagers * scores  # Detach scores to prevent gradients flowing through them (treat as constant for routing)
            net_payout = sigmoid_wagers * (brier_scores.sum(dim=1, keepdim=True)/(brier_scores.shape[1] - 1) - brier_scores)
