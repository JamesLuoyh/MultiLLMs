...
class ScoringFunction:
    """
    Helper class containing various scoring functions for evaluating model predictions.
    
    Each scoring function takes:
    - ground_truth: [batch_size] - indices of correct options
    - predictions: [batch_size, num_models, num_options] - logits/probabilities for each option
    - wagers: [batch_size, num_models] - model wagers (not used in scoring, included for compatibility)
    
    Returns:
    - scores: [batch_size, num_models] - score for each model in each sample
    """
    
    @staticmethod
    def linear_score(ground_truth: torch.Tensor, predictions: torch.Tensor, wagers: torch.Tensor = None) -> torch.Tensor:
        """
        Linear scoring: return the probability assigned to the correct option.
        If ground truth is option A, return p(A).
        
        Args:
            ground_truth: [batch_size] - indices of correct options
            predictions: [batch_size, num_models, num_options] - logits for each option
            wagers: [batch_size, num_models] - not used, included for API consistency
            
        Returns:
            scores: [batch_size, num_models] - probability of correct option
        """
        # Convert logits to probabilities
        probs = F.softmax(predictions, dim=-1)  # [batch_size, num_models, num_options]
        
        batch_size, num_models, num_options = probs.shape
        
        # Gather the probability of the correct option for each model
        # ground_truth: [batch_size] -> [batch_size, 1, 1]
        gt_expanded = ground_truth.view(batch_size, 1, 1).expand(batch_size, num_models, 1)
        
        # Gather: [batch_size, num_models, 1]
        correct_probs = torch.gather(probs, dim=2, index=gt_expanded)
        
        # Remove last dimension: [batch_size, num_models]
        scores = correct_probs.squeeze(-1)
        
        return scores
    

    @staticmethod
    def custom_score(ground_truth: torch.Tensor, predictions: torch.Tensor, wagers: torch.Tensor = None) -> torch.Tensor:
        """
        Brier scoring: negative mean squared error between predictions and one-hot ground truth.
        Lower MSE is better, so we return negative MSE (higher is better).
        
        Brier score = -sum((p_i - y_i)^2) where y_i is 1 for correct option, 0 otherwise.
        ...
        Returns:
            scores: [batch_size, num_models] - negative Brier score (higher is better)
        """
        # Convert logits to probabilities
        probs = F.softmax(predictions, dim=-1)  # [batch_size, num_models, num_options]
        ...
        gt_onehot = F.one_hot(ground_truth, num_classes=num_options).float()  # [batch_size, num_options]
        ...
        scores = 2-brier_scores# - 0.0005 * wagers  # Custom modification
        
        return scores
    @staticmethod
    def normalized_linear_score(ground_truth: torch.Tensor, predictions: torch.Tensor, wagers: torch.Tensor = None) -> torch.Tensor:
        """
        Linear scoring: return the probability assigned to the correct option.
        If ground truth is option A, return p(A).
        ...
        # Remove last dimension: [batch_size, num_models]
        aggregated_probs = (wagers.unsqueeze(-1) * correct_probs).sum(dim=1, keepdim=True)
        
        scores = correct_probs.squeeze(-1) / aggregated_probs.squeeze(-1)

        return scores

    @staticmethod
    def log_score(ground_truth: torch.Tensor, predictions: torch.Tensor, wagers: torch.Tensor = None) -> torch.Tensor:
        """
        Log scoring: return the log probability assigned to the correct option.
        If ground truth is option A, return log(p(A)).
        ...
        Returns:
            scores: [batch_size, num_models] - log probability of correct option
        """
        # Convert logits to log probabilities
        log_probs = F.log_softmax(predictions, dim=-1)  # [batch_size, num_models, num_options]
        
        batch_size, num_models, num_options = log_probs.shape
        
        # Gather the log probability of the correct option for each model
        gt_expanded = ground_truth.view(batch_size, 1, 1).expand(batch_size, num_models, 1)
        
        # Gather: [batch_size, num_models, 1]
        correct_log_probs = torch.gather(log_probs, dim=2, index=gt_expanded)
        
        # Remove last dimension: [batch_size, num_models]
        scores = correct_log_probs.squeeze(-1)
        
        return scores
    
    @staticmethod
    def brier_score(ground_truth: torch.Tensor, predictions: torch.Tensor, wagers: torch.Tensor = None) -> torch.Tensor:
        """
        Brier scoring: negative mean squared error between predictions and one-hot ground truth.
        Lower MSE is better, so we return negative MSE (higher is better).
    ...
    @classmethod
    def get_scoring_function(cls, name: str):
        """
        Get a scoring function by name.
        
        Args:
            name: One of 'linear', 'log', 'brier', 'normalized_linear'
            
        Returns:
            Scoring function
        """
        scoring_functions = {
            'linear': cls.linear_score,
            'log': cls.log_score,
            'brier': cls.brier_score,
            'normalized_linear': cls.normalized_linear_score,
            'custom_score': cls.custom_score
        }
        
        if name not in scoring_functions:
            raise ValueError(f"Unknown scoring function: {name}. Must be one of {list(scoring_functions.keys())}")
        
        return scoring_functions[name]


class WeightedScoreWagers(WageringMethod):
    """
    ...
    def __init__(self, num_models: int, config: Optional[Dict[str, Any]] = None):
        ...
        self.scoring_function = ScoringFunction.get_scoring_function(self.score_function_name)
    ...
    def update(
        ...
        **kwargs
        ...
        for i in range(self.num_models):
            ...
            hs_tensor.requires_grad_(False)
        ...
        with torch.enable_grad():
            ...
            raw_wagers_list = []
            for i in range(self.num_models):
                model_hs_i = hidden_states_tensors[i]
                
                # Apply projection if exists
                proj_key = f"proj_{i}"
                if proj_key in self.model_projections:
                    projection_i = self.model_projections[proj_key]
                    model_projected_i = projection_i(model_hs_i)
                else:
                    model_projected_i = model_hs_i
                
                # Normalize if needed
                if self.normalize_hidden_states:
                    norms = torch.norm(model_projected_i, dim=-1, keepdim=True)
                    model_projected_i = model_projected_i / (norms + 1e-8)
                
                # Apply router
                raw_wager_i = self.routers[i](model_projected_i)
                raw_wagers_list.append(raw_wager_i)
            
                raw_wagers_tensor = torch.cat(raw_wagers_list, dim=1)
                sigmoid_wagers = torch.sigmoid(raw_wagers_tensor / self.temperature) # [batch_size, num_models]
                wagers_all = sigmoid_wagers / (torch.sum(sigmoid_wagers, dim=1, keepdim=True) + 1e-10)
                ...
                # Sum over options (Brier score): [batch_size, num_models]
                brier_scores = squared_errors.sum(dim=-1).detach()  # Detach to prevent gradients flowing through scores (treat as constant for routing)
                
                # Return negative (since lower Brier score is better, but we want higher scores to be better)
                scores = 2-brier_scores - 0.5 * (sigmoid_wagers)  # Custom modification
                    

                # print((scores * sigmoid_wagers))
                # print("-"*20)
                # print(sigmoid_wagers.sum(dim=1, keepdim=True))
                # scores = (scores - ((scores * sigmoid_wagers).sum(dim=1, keepdim=True)) / (sigmoid_wagers.sum(dim=1, keepdim=True)))#.detach()
                average_scores = ((scores * sigmoid_wagers).sum(dim=1, keepdim=True).expand_as(scores * sigmoid_wagers)) / (sigmoid_wagers.sum(dim=1, keepdim=True).expand_as(sigmoid_wagers))
                # print(scores)
                # print("average_scores", average_scores)
                scores = scores - average_scores.detach()
                net_payout = sigmoid_wagers * scores # Detach scores to prevent gradients flowing through them (treat as constant for routing)
                # print("scores", scores)
                # print("sigmoid_wagers", sigmoid_wagers)
                # print("net_payout", net_payout)
                # 0/0
                ...
                loss_i = -net_payout[:, i].mean() # losses.mean()  #
